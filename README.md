...
| **Rank Math** (WP plugin) | attributes `Rank Math` via `wp_redirect()` 3rd arg (filterable) | ✅ Automatic once core is patched — no change needed | [`class-redirector.php`](https://github.com/rankmath/seo-by-rank-math/blob/master/includes/modules/redirections/class-redirector.php) |
| **All in One SEO** (WP plugin) | attributes `All in One SEO` via `wp_safe_redirect()` 3rd arg | ✅ Automatic once core is patched — no change needed | [`Rss.php`](https://github.com/awesomemotive/all-in-one-seo-pack/blob/main/app/Common/Rss.php) |
| **Yoast SEO** (WP plugin) | free: attributes `Yoast SEO` via `redirect-helper.php` 3rd arg; Premium redirect module sets `Yoast SEO Premium` | ✅ Free auto once core is patched; Premium is an internal change | [`redirect-helper.php`](https://github.com/Yoast/wordpress-seo/blob/trunk/src/helpers/redirect-helper.php) |
